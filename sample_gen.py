...
def BornSampler(N, N_v,
	...
	born_samples = np.flip(np.delete(born_samples_all_qubits, range(N_v, 5), axis=1), 1)
...
def PlusMinusSampleGen(N, N_v,
	...
	born_samples_plus_all_qbs_dict = qc.run_and_measure(prog_plus, N_born_samples_plus)
	#All (5) qubits are measured at once
	born_samples_plus_all_qbs = np.vstack(born_samples_plus_all_qbs_dict[q] for q in sorted(qc.qubits())).T
	#Remove unused qubits and flip results for Rigetti convention |001> written as |100>
	born_samples_plus = np.flip(np.delete(born_samples_plus_all_qbs, range(N_v, 5), axis=1), 1)


	born_samples_plus_all_qbs_dict = qc.run_and_measure(prog_minus, N_born_samples_minus)
	#All (5) qubits are measured at once in QPU (and QVM in run_and_measure)
	born_samples_minus_all_qbs = np.vstack(born_samples_plus_all_qbs_dict[q] for q in sorted(qc.qubits())).T
	#Remove unused qubits and flip results for Rigetti convention |001> written as |100>
	born_samples_minus = np.flip(np.delete(born_samples_minus_all_qbs, range(N_v, 5), axis=1), 1)
